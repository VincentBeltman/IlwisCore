...
Modeller.ModellerWorkArea {
    ...
    function asignConstantInputData(inputData, itemId) {
        var parameterIndexes = workflow.asignConstantInputData(inputData,
                                                               itemId)
        ...
        onYes: {
            ...
            for (var i = 0; i < wfCanvas.operationsList.length; i++) {
                ...
                for (var j = 0; j < deleteFlows.length; j++) {
                    wfCanvas.operationsList[i].flowConnections.splice(
                                deleteFlows[j] - j, 1)
                }
            }
    ...
    MessageDialog {
        ...
        onYes: {
            ...
            var parameterIndexes = workflow.deleteFlow(from, to, outputIndex,
                                                       inputIndex)
            wfCanvas.operationsList[deleteItemIndex].flowConnections.splice(
                        deleteEdgeIndex, 1)
            flow.target.resetInputModel()
            wfCanvas.canvasValid = false
    ...
    Canvas {
        ...
        function draw(force) {
            ...
            if (canvasValid == false || (force !== null && force)) {
                ...
                if (workingLineBegin.x !== -1 && workingLineEnd.x !== -1) {
                    ctx.beginPath()
                    ctx.lineWidth = 2
                    var pt1 = transformedPoint(workingLineBegin.x,
                                               workingLineBegin.y)
                    var pt2 = transformedPoint(workingLineEnd.x,
                                               workingLineEnd.y)
        ...
        function finishCreation(x, y, resource) {
            if (component.status == Component.Ready) {
                currentItem = component.createObject(wfCanvas, {
                                                         x: x,
                                                         y: y,
                                                         operation: resource,
                                                         itemid: operationsList.length,
                                                         scale: wfCanvas.scale
                                                     })
                if (currentItem == null) {
                    // Error Handling
        ...
        function isInsideCondition(centreX, centreY) {
            for (var i = 0; i < wfCanvas.conditionBoxList.length; i++) {
                ...
                if (centreX > box.x && centreY > box.y
                        && centreX < (box.x + box.width)
                        && centreY < (box.y + box.height)) {
        ...
        DropArea {
            ...
            onDropped: {
                if (drag.source.type === "singleoperation"
                    ...
                    var paramterIndexes = workflow.addOperation(
                                drag.source.ilwisobjectid)

                    var item = wfCanvas.currentItem
                    wfCanvas.isInsideCondition(item.x + (item.width / 2),
        ...
        MouseArea {
            ...
            onPressed: {
                if (canvasActive) {
                    ...
                    var operationSelected = -1, highestZ = -1, smallestDistance = 100000, selectedFlow = false, implicitIndexes, constantValues
                    ...
                    for (var i = 0; i < wfCanvas.operationsList.length; ++i) {
                        ...
                        var isContained = mouseX >= (startCoords.x)
                                && mouseY >= (startCoords.y) && mouseX <= endX
                                && mouseY <= endY
                        ...
                        for (var j = 0; j < item.flowConnections.length; j++) {
                            ...
                            var distanceAC = Math.sqrt(Math.pow((ax - mouseX),
                                                                2) + Math.pow(
                                                           (ay - mouseY), 2))
                            var distanceBC = Math.sqrt(Math.pow((bx - mouseX),
                                                                2) + Math.pow(
                                                           (by - mouseY), 2))
                            var distanceAB = Math.sqrt(Math.pow((ax - bx),
                                                                2) + Math.pow(
                                                           (ay - by), 2))
                    ...
                    if (!selectedFlow && operationSelected == -1) {
                        ...
                        wfCanvas.dragStart = transformedPoint(wfCanvas.lastX,
                                                              wfCanvas.lastY)
                    ...
                    if (selectedFlow && operationSelected == -1) {
                        ...
                        implicitIndexes = workflow.implicitIndexes(
                                    operationSelected)
                        constantValues = workflow.getAsignedValuesByItemID(
                                    operationSelected)

                        if (implicitIndexes) {
                            manager.showOperationFormWithHiddenFields(
                                        item, operationSelected,
                                        constantValues, implicitIndexes)
                        } else {
            ...
            onDoubleClicked: {
                ...
                for (var i = 0; i < wfCanvas.operationsList.length; ++i) {
                    ...
                    var isContained = mouseX >= item.x && mouseY >= item.y
                ...
                if (operationSelected > -1) {
                    ...
                    bigthing.newCatalog(filter, "workflow",
                                        resource.url, "other")
                }
            }
